...
export function PresenterPortalHomePage() {
  ...
  const avisoPendente = pendentes ? `${pendentes} envio${pendentes > 1 ? 's' : ''} pendente${pendentes > 1 ? 's' : ''} de aprovação (${formatMoney(gmvPendente, true)}). Comissão não incluída.` : 'Registradas e aprovadas no período.'
  return <div className="space-y-6"><PageHeader title={`Olá, ${perfil.nome.split(' ')[0]}`} subtitle="Acompanhe o desempenho das suas lives concluídas e envios pendentes." actions={<label className="grid gap-1 text-xs font-semibold text-ink-muted">Mês<input aria-label="Mês do desempenho" className="design-input h-10 px-3 text-sm" type="month" value={mes} onChange={(e) => setMes(e.target.value)} /></label>} />
    <div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4"><Metric icon={Video} label="Lives" value={String(desempenho.total_lives)} hint={avisoPendente} /><Metric icon={Clock3} label="Horas" value={`${desempenho.horas_live.toLocaleString('pt-BR', { maximumFractionDigits: 1 })}h`} hint="Horas atribuídas conforme a divisão da live." /><Metric icon={CircleDollarSign} label="GMV" value={formatMoney(desempenho.gmv_lives, true)} hint={pendentes ? 'Inclui GMV pendente; comissão permanece aguardando validação.' : 'Resultado atribuído à sua participação.'} /><Metric icon={BarChart3} label="GMV por hora" value={desempenho.gmv_por_hora === null ? '—' : formatMoney(desempenho.gmv_por_hora, true)} hint="GMV atribuído dividido pelas suas horas." /></div>
