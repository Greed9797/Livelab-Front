...
function Metric({ icon: Icon, label, value, hint }: { icon: typeof Video; label: string; value: string; hint: string }) {
  return <Card><CardBody className="min-w-0 p-3 sm:p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-ink-muted">{label}</p><p className="num mt-2 break-words text-lg sm:text-2xl font-black tracking-[-0.03em] text-ink">{value}</p></div><span className="hidden h-10 w-10 shrink-0 place-items-center sm:grid rounded-xl bg-surface-muted text-ink-muted"><Icon className="h-5 w-5" /></span></div><p className="mt-3 text-sm text-ink-muted">{hint}</p></CardBody></Card>
}
...
export function PresenterPortalHomePage() {
  ...
  const pendentes = desempenho.total_lives_pendentes_aprovacao ?? 0
  const gmvPendente = desempenho.gmv_pendente_aprovacao ?? 0
  const avisoPendente = desempenho.em_conciliacao ? `Em conciliação: ${formatMoney(gmvPendente, true)} declarado pendente. Confira o vínculo em Minhas lives; não foi somado ao subtotal.` : pendentes ? `${pendentes} envio${pendentes > 1 ? 's' : ''} pendente${pendentes > 1 ? 's' : ''} de aprovação (${formatMoney(gmvPendente, true)}). Comissão não incluída.` : 'Registradas e aprovadas no período.'
  return <div className="space-y-6"><PageHeader title={`Olá, ${perfil.nome.split(' ')[0]}`} subtitle="Acompanhe o desempenho das suas lives concluídas e envios pendentes." actions={<label className="grid gap-1 text-xs font-semibold text-ink-muted">Mês<input aria-label="Mês do desempenho" className="design-input h-10 px-3 text-sm" type="month" value={mes} onChange={(e) => setMes(e.target.value)} /></label>} />
    <div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4"><Metric icon={Video} label="Lives" value={String(desempenho.total_lives)} hint={avisoPendente} /><Metric icon={Clock3} label="Horas" value={`${desempenho.horas_live.toLocaleString('pt-BR', { maximumFractionDigits: 1 })}h`} hint="Horas atribuídas conforme a divisão da live." /><Metric icon={CircleDollarSign} label="GMV" value={formatMoney(desempenho.gmv_lives, true)} hint={desempenho.em_conciliacao ? 'Subtotal sem os envios em conciliação; não é o total consolidado.' : pendentes ? 'Total provisório; comissão permanece aguardando validação.' : 'Resultado atribuído à sua participação.'} /><Metric icon={BarChart3} label="GMV por hora" value={desempenho.gmv_por_hora === null ? '—' : formatMoney(desempenho.gmv_por_hora, true)} hint="GMV atribuído dividido pelas suas horas." /></div>
    <p className="rounded-xl border border-line p-3 text-sm text-ink-muted">GMV validado: {formatMoney(desempenho.gmv_validado ?? desempenho.gmv_lives, true)} · Pendente aprovação: {formatMoney(gmvPendente, true)} · {desempenho.em_conciliacao ? 'Em conciliação — total consolidado indisponível' : `Total provisório: ${formatMoney(desempenho.total_provisorio ?? desempenho.gmv_lives, true)}`}</p><div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.8fr)]"><PortalRanking rows={ranking} /><Card><CardBody className="p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-surface-muted text-ink-muted"><ShoppingBag className="h-5 w-5" /></span><div><p className="text-base font-bold text-ink">Apuração do mês</p><p className="text-sm text-ink-muted">Valores calculados para a competência selecionada.</p></div></div><p className="num mt-6 text-3xl font-black tracking-[-0.03em] text-ink">{remuneracao.total === null || remuneracao.total === undefined ? 'Não informado' : formatMoney(remuneracao.total, true)}</p><dl className="mt-5 grid gap-2 border-t border-line pt-4 text-sm"><div className="flex justify-between gap-3"><dt className="text-ink-muted">Fixo</dt><dd className="num font-semibold text-ink">{remuneracao.fixo === null ? '—' : formatMoney(remuneracao.fixo, true)}</dd></div><div className="flex justify-between gap-3"><dt className="text-ink-muted">Comissão</dt><dd className="num font-semibold text-ink">{remuneracao.comissao === null || remuneracao.comissao === undefined ? '—' : formatMoney(remuneracao.comissao, true)}</dd></div><div className="flex justify-between gap-3"><dt className="text-ink-muted">Extras</dt><dd className="num font-semibold text-ink">{remuneracao.adicionais === null || remuneracao.adicionais === undefined ? '—' : formatMoney(remuneracao.adicionais, true)}</dd></div></dl>{remuneracao.extras?.length ? <div className="mt-4 border-t border-line pt-4"><p className="text-xs font-bold uppercase tracking-[0.1em] text-ink-muted">Detalhes dos extras</p><ul className="mt-2 space-y-2 text-sm">{remuneracao.extras.map((extra) => <li key={extra.id} className="flex justify-between gap-3"><span className="min-w-0 truncate text-ink-muted">{extra.descricao}</span><span className="num shrink-0 font-semibold text-ink">{formatMoney(extra.valor, true)}</span></li>)}</ul></div> : null}<p className="mt-4 text-sm leading-6 text-ink-muted">A apuração não confirma pagamento; o fechamento é feito pela gestão.</p></CardBody></Card></div>
  </div>
}
