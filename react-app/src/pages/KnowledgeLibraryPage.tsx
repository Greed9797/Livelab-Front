...
function CategoryManager({ open, categories, busy, onClose, onCreate, onUpdate, onToggle, onReorder }: { open: boolean; categories: KnowledgeCategory[]; busy?: boolean; onClose: () => void; onCreate: (name: string) => Promise<void>; onUpdate: (id: string, name: string) => Promise<void>; onToggle: (category: KnowledgeCategory) => Promise<void>; onReorder: (ids: string[]) => Promise<void> }) {
  ...
  return <Modal open={open} title="Categorias da unidade" subtitle="Edite, desative ou reorganize os filtros da Base." onClose={onClose} size="lg" footer={<div className="flex justify-end"><Button type="button" variant="ghost" icon={X} onClick={onClose}>Fechar</Button></div>}><div className="space-y-4"><form className="flex gap-2" onSubmit={(event) => { event.preventDefault(); void create() }}><input aria-label="Nova categoria" className="design-input h-10 min-w-0 flex-1 px-3" value={name} onChange={(event) => setName(event.target.value)} placeholder="Nova categoria" maxLength={120} /><Button type="submit" icon={FolderPlus} isLoading={busyId === 'new'}>Criar</Button></form>{categories.length ? <div className="space-y-2">{categories.map((category, index) => <div key={category.id} className="flex flex-wrap items-center gap-2 rounded-xl border border-line bg-surface-muted p-2"><input aria-label={`Nome da categoria ${category.name}`} className="design-input h-9 min-w-0 flex-1 px-2 text-sm" value={drafts[category.id] ?? category.name} onChange={(event) => setDrafts((current) => ({ ...current, [category.id]: event.target.value }))} /><Button type="button" size="icon" variant="ghost" aria-label={`Salvar categoria ${category.name}`} onClick={() => void update(category)} disabled={busyId === category.id || busy}><Save className="h-4 w-4" /></Button><Button type="button" size="icon" variant="ghost" aria-label={`Mover ${category.name} para cima`} onClick={() => void move(index, -1)} disabled={index === 0 || busyId === 'order' || busy}><ArrowUp className="h-4 w-4" /></Button><Button type="button" size="icon" variant="ghost" aria-label={`Mover ${category.name} para baixo`} onClick={() => void move(index, 1)} disabled={index === categories.length - 1 || busyId === 'order' || busy}><ArrowDown className="h-4 w-4" /></Button><Button type="button" size="icon" variant="ghost" aria-label={category.is_active === false ? `Reativar ${category.name}` : `Desativar ${category.name}`} onClick={() => void toggle(category)} disabled={busyId === category.id || busy}>{category.is_active === false ? <Check className="h-4 w-4" /> : <X className="h-4 w-4" />}</Button></div>)}</div> : <p className="text-sm text-ink-muted">Nenhuma categoria cadastrada.</p>}</div></Modal>
}
...
export function KnowledgeLibraryPage() {
  ...
  if (articleRef) {
    ...
    return <LocalReader material={detail.data} manager={manager} headingRef={articleHeading} onBack={closeLocal} onEdit={() => openEdit(detail.data!)} />
  }
...
function LocalReader({ material, manager, headingRef, onBack, onEdit }: { material: KnowledgeMaterial; manager: boolean; headingRef: React.RefObject<HTMLHeadingElement | null>; onBack: () => void; onEdit: () => void }) {
