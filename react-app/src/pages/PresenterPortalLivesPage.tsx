...
export function payloadFromForm(form: FormState): PresenterSubmissionPayload | null {
  ...
  if (!form.marcaId || !form.dia || !form.horaInicio || !form.horaFim || !form.observacao.trim() || !gmv.ok || !pedidos.ok || !liveImpressions.ok || !manualViews.ok) return null
...
function SubmissionModal({ item, options, optionsLoading, optionsError, errorMessage, onClose, onSave, busy }: { item: PresenterSubmission | null; options?: PresenterPortalOptions; optionsLoading: boolean; optionsError: unknown; errorMessage?: string; onClose: () => void; onSave: (payload: PresenterSubmissionPayload) => void; busy: boolean }) {
  ...
  const maxDate = new Date(Number(month.slice(0, 4)), Number(month.slice(5, 7)), 0).toISOString().slice(0, 10)
  const minDate = `${month}-01`
  return <Modal open title={item ? 'Corrigir envio' : 'Registrar live concluída'} subtitle="Obrigatórios: marca, data, horários, impressões, visualizações, GMV, pedidos e observação." onClose={onClose} closeDisabled={busy} footer={<div className="flex w-full flex-wrap justify-end gap-2"><Button variant="secondary" onClick={onClose} disabled={busy}>Cancelar</Button><Button icon={Send} isLoading={busy} disabled={unavailable} onClick={() => { const payload = payloadFromForm(form); if (!payload) { setError('Preencha os campos obrigatórios. Use somente números válidos; zero é permitido. A live deve terminar depois do início, no mesmo dia deste mês.'); return }; onSave({ ...payload, request_id: item ? undefined : requestId.current }) }}>{item ? 'Salvar correção' : 'Enviar para revisão'}</Button></div>}>
    <div className="grid gap-4 p-1 sm:grid-cols-2">
      <label className="grid gap-1.5 text-sm font-semibold text-ink sm:col-span-2">Marca<select className="design-input h-11 px-3" value={form.marcaId} onChange={(e) => set('marcaId', e.target.value)} disabled={unavailable} required><option value="">{optionsLoading ? 'Carregando marcas…' : optionsError ? 'Não foi possível carregar marcas' : 'Selecione a marca'}</option>{options?.marcas.map((marca) => <option key={marca.id} value={marca.id}>{marca.nome}</option>)}</select></label>
      ...
      <label className="grid gap-1.5 text-sm font-semibold text-ink sm:col-span-2">Dia<input aria-label="Dia da live" className="design-input h-11 px-3" type="date" min={minDate} max={maxDate} value={form.dia} onChange={(e) => set('dia', e.target.value)} required /></label>
      <label className="grid gap-1.5 text-sm font-semibold text-ink">Hora de início<input aria-label="Hora de início" className="design-input h-11 px-3" type="time" value={form.horaInicio} onChange={(e) => set('horaInicio', e.target.value)} required /></label>
      <label className="grid gap-1.5 text-sm font-semibold text-ink">Hora de fim<input aria-label="Hora de fim" className="design-input h-11 px-3" type="time" value={form.horaFim} onChange={(e) => set('horaFim', e.target.value)} required /></label>
      <label className="grid gap-1.5 text-sm font-semibold text-ink">GMV declarado<input aria-label="GMV declarado" className="design-input h-11 px-3" inputMode="decimal" placeholder="Ex.: 1.234,56" value={form.gmv} onChange={(e) => set('gmv', e.target.value)} required /></label>
      <label className="grid gap-1.5 text-sm font-semibold text-ink">Pedidos<input aria-label="Pedidos declarados" className="design-input h-11 px-3" inputMode="numeric" value={form.pedidos} onChange={(e) => set('pedidos', e.target.value)} required /></label>
      <label className="grid gap-1.5 text-sm font-semibold text-ink">Impressões<input aria-label="Impressões da live" className="design-input h-11 px-3" inputMode="numeric" value={form.liveImpressions} onChange={(e) => set('liveImpressions', e.target.value)} required /></label>
      <label className="grid gap-1.5 text-sm font-semibold text-ink">Visualizações<input aria-label="Visualizações" className="design-input h-11 px-3" inputMode="numeric" value={form.manualViews} onChange={(e) => set('manualViews', e.target.value)} required /></label>
      <label className="grid gap-1.5 text-sm font-semibold text-ink sm:col-span-2">Observação<textarea className="design-input min-h-24 p-3" value={form.observacao} onChange={(e) => set('observacao', e.target.value)} maxLength={2000} required /></label>
      {options?.cabines.length ? <details className="rounded-xl border border-line p-3 text-sm text-ink-muted sm:col-span-2"><summary className="cursor-pointer font-semibold text-ink">Mais informações (opcional)</summary><label className="mt-3 grid gap-1.5 font-semibold text-ink">Cabine<select className="design-input h-11 px-3" value={form.cabineId} onChange={(e) => set('cabineId', e.target.value)}><option value="">Não informada</option>{options.cabines.map((cabine) => <option key={cabine.id} value={cabine.id}>{cabine.nome || `Cabine ${cabine.numero ?? ''}`}</option>)}</select></label></details> : null}
