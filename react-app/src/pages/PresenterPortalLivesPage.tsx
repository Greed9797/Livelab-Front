...
import { respondPresenterArchive, cancelPresenterSubmission, createPresenterSubmission, getPresenterPortalLives, getPresenterPortalOptions, resubmitPresenterSubmission, updatePresenterSubmission, type PresenterPortalLive, type PresenterPortalOptions, type PresenterSubmission, type PresenterSubmissionPayload } from '../services/presenter-portal'
import './PresenterPortalLivesPage.css'
import { QK } from '../services/query-keys'
import { useCurrentUser } from '../stores/auth-store'
import { extractErrorMessage } from '../services/api'
import { formatDate, formatMoney } from '../utils/format'
import { currentMonth, isoFromLocalDateTime, localDateTimeValue, submissionHasOfficialLiveTombstone, submissionOwnerCanCancel, submissionStatusLabel, submissionStatusTone } from '../utils/presenter-portal'
...
type LiveStatusBit = { label: string; tone: BadgeTone }

export function completedLiveStatusBits(item: Pick<PresenterPortalLive, 'uniao_id' | 'pendente_aprovacao' | 'em_conciliacao'>): LiveStatusBit[] {
  const bits: LiveStatusBit[] = []
  if (item.uniao_id) bits.push({ label: 'Transmissão unida · sua participação', tone: 'info' })
  if (item.pendente_aprovacao) bits.push({ label: item.em_conciliacao ? 'Pendente aprovação · conferir vínculo' : 'Pendente aprovação', tone: 'warning' })
  return bits
}

export function submissionMobileStatusBits(item: Pick<PresenterSubmission, 'status' | 'arquivamento_status' | 'live_oficial_excluida_id' | 'live_oficial_excluida_em'>): LiveStatusBit[] {
  const label = item.arquivamento_status === 'confirmado' ? 'Arquivado' : item.arquivamento_status === 'solicitado' ? 'Aguardando sua resposta' : submissionStatusLabel(item.status)
  const tone: BadgeTone = item.arquivamento_status === 'solicitado' ? 'warning' : item.arquivamento_status === 'confirmado' ? 'neutral' : submissionStatusTone(item.status)
  const bits: LiveStatusBit[] = [{ label, tone }]
  if (submissionHasOfficialLiveTombstone(item)) bits.push({ label: 'Live excluída pelo gestor', tone: 'neutral' })
  return bits
}

export function presenterLivesAttention(items: Array<Pick<PresenterSubmission, 'status' | 'arquivamento_status'>>): { label: string; tone: 'warning' | 'danger' } | null {
  const waiting = items.some((item) => item.arquivamento_status === 'solicitado')
  const correction = items.some((item) => item.status === 'devolvida' && item.arquivamento_status !== 'solicitado' && item.arquivamento_status !== 'confirmado')
  if (!waiting && !correction) return null
  const parts = [waiting ? 'Aguardando sua resposta' : null, correction ? 'Devolvida para ajuste' : null].filter((part): part is string => Boolean(part))
  return { label: parts.join(' · '), tone: waiting ? 'warning' : 'danger' }
}

function participationMeta(item: PresenterPortalLive): string {
  const parts = [formatDate(item.iniciado_em), `${item.horas.toLocaleString('pt-BR', { maximumFractionDigits: 1 })}h`, `${item.pedidos} pedidos`]
  if (item.cabine_nome) parts.push(item.cabine_nome)
  return parts.join(' · ')
}

function StatusMeta({ bits }: { bits: LiveStatusBit[] }) {
  return <>{bits.map((bit) => <span key={bit.label} className={`presenter-lives-status presenter-lives-status--${bit.tone}`}> · {bit.label}</span>)}</>
}

function CompletedLivesMobile({ items }: { items: PresenterPortalLive[] }) {
  return <div className="presenter-lives-mobile">{items.map((item) => {
    const bits = completedLiveStatusBits(item)
    const meta = participationMeta(item)
    return <div key={item.id} className="presenter-lives-row" title={[meta, ...bits.map((bit) => bit.label)].join(' · ')}><div className="presenter-lives-row__main"><p className="presenter-lives-row__marca">{item.marca_nome ?? 'Marca não informada'}</p><p className="presenter-lives-row__meta"><span className="presenter-lives-row__lead">{meta}</span>{bits.length ? <span className="presenter-lives-row__statuses"><StatusMeta bits={bits} /></span> : null}</p></div><span className="presenter-lives-row__gmv num">{formatMoney(item.gmv, true)}</span></div>
  })}</div>
}

function SubmissionMobileList({ items, resendPending, onConfirm, onContest, onEdit, onResend, onCancel }: { items: PresenterSubmission[]; resendPending: boolean; onConfirm: (item: PresenterSubmission) => void; onContest: (item: PresenterSubmission) => void; onEdit: (item: PresenterSubmission) => void; onResend: (id: string) => void; onCancel: (item: PresenterSubmission) => void }) {
  return <div className="presenter-lives-mobile">{items.map((item) => {
    const bits = submissionMobileStatusBits(item)
    const reason = item.motivo_devolucao?.trim()
    const waiting = item.arquivamento_status === 'solicitado'
    const returned = !waiting && item.status === 'devolvida'
    return <div key={item.id} className="presenter-lives-envio"><div className="presenter-lives-row" title={[formatDate(item.iniciado_em), ...bits.map((bit) => bit.label), reason].filter(Boolean).join(' · ')}><div className="presenter-lives-row__main"><p className="presenter-lives-row__marca">{item.marca_nome ?? item.marca_descricao ?? 'Marca'}</p><p className="presenter-lives-row__meta"><span className="presenter-lives-row__lead">{formatDate(item.iniciado_em)}</span><span className="presenter-lives-row__statuses"><StatusMeta bits={bits} /></span>{reason ? <span className="presenter-lives-row__reason"> · {reason}</span> : null}</p></div></div>{waiting ? <div className="presenter-lives-actions"><Button variant="secondary" aria-label="Confirmar arquivamento" onClick={() => onConfirm(item)}>Confirmar</Button><Button aria-label="Contestar devolução" onClick={() => onContest(item)}>Contestar</Button></div> : null}{returned ? <div className="presenter-lives-actions"><Button variant="secondary" aria-label="Corrigir" onClick={() => onEdit(item)}>Corrigir</Button><Button aria-label="Reenviar" isLoading={resendPending} onClick={() => onResend(item.id)}>Reenviar</Button><Button variant="ghost" aria-label="Cancelar envio" onClick={() => onCancel(item)}>Cancelar</Button></div> : null}{item.status === 'pendente' && submissionOwnerCanCancel(item) ? <div className="presenter-lives-actions"><Button variant="ghost" aria-label="Cancelar envio" onClick={() => onCancel(item)}>Cancelar</Button></div> : null}</div>
  })}</div>
}
...
export function PresenterPortalLivesPage() {
  ...
  const officialSlice = query.data.items.slice(officialPage * 10, officialPage * 10 + 10)
  const submissionSlice = visibleSubmissions.slice(submissionPage * 10, submissionPage * 10 + 10)
  const attention = presenterLivesAttention(query.data.submissoes)
  return <div className="presenter-lives space-y-6"><PageHeader title="Minhas lives" subtitle="Envios entram nas métricas como pendentes; comissão só após validação." actions={<><label className="grid gap-1 text-xs font-semibold text-ink-muted">Mês<input aria-label="Mês das minhas lives" className="design-input h-10 px-3 text-sm" type="month" value={mes} onChange={(e) => setMes(e.target.value)} /></label><Button icon={ClipboardPlus} disabled={!canRegisterCurrentMonth} title={canRegisterCurrentMonth ? undefined : 'O cadastro é permitido somente no mês atual'} onClick={() => setEditing(null)}>Registrar live</Button></>} />
    {attention ? <a href="#meus-envios" className={attention.tone === 'danger' ? 'presenter-lives-alert presenter-lives-alert--danger' : 'presenter-lives-alert'} aria-label={`${attention.label}. Ir para Meus envios`} onClick={(event) => { event.preventDefault(); const index = visibleSubmissions.findIndex((item) => item.arquivamento_status === 'solicitado' || (item.status === 'devolvida' && item.arquivamento_status !== 'confirmado')); if (index >= 0) setSubmissionPage(Math.floor(index / 10)); requestAnimationFrame(() => document.getElementById('meus-envios')?.scrollIntoView({ block: 'start' })) }}><span className="presenter-lives-alert__text">{attention.label}</span></a> : null}
    <Card><CardHeader><h2 className="text-lg font-bold text-ink">Lives concluídas</h2><p className="mt-1 text-sm text-ink-muted">GMV, horas e pedidos mostram somente a sua participação em cada transmissão.</p></CardHeader><CardBody className="p-0">{query.data.items.length === 0 ? <EmptyState title="Nenhuma live concluída no período" /> : <><div className="presenter-lives-desktop"><DataTable data={officialSlice} rowKey={(item) => item.id} columns={[{ key: 'marca', header: 'Marca e data', render: (item) => <div><p className="font-semibold text-ink">{item.marca_nome ?? 'Marca não informada'}</p><div className="mt-1 flex flex-wrap gap-1.5">{item.uniao_id ? <Badge tone="info">Transmissão unida · sua participação</Badge> : null}{item.pendente_aprovacao ? <Badge tone="warning">{item.em_conciliacao ? 'Pendente aprovação · conferir vínculo' : 'Pendente aprovação'}</Badge> : null}</div><p className="text-xs text-ink-muted">{formatDate(item.iniciado_em)} · {item.cabine_nome ?? 'Cabine não informada'}</p></div> }, { key: 'gmv', header: 'Seu GMV', align: 'right', render: (item) => <span className="num font-bold">{formatMoney(item.gmv, true)}</span> }, { key: 'resultado', header: 'Sua participação', align: 'right', render: (item) => <span className="text-ink-muted">{item.horas.toLocaleString('pt-BR', { maximumFractionDigits: 1 })}h · {item.pedidos} pedidos</span> }]} /></div><CompletedLivesMobile items={officialSlice} /><TablePager page={officialPage} total={query.data.items.length} onChange={setOfficialPage} /></>}</CardBody></Card>
    <Card id="meus-envios" className="presenter-lives-submissions"><CardHeader><h2 className="text-lg font-bold text-ink">Meus envios</h2><p className="mt-1 text-sm text-ink-muted">Acompanhe a revisão e corrija somente envios devolvidos.</p><label className="mt-3 flex items-center gap-2 text-sm"><input type="checkbox" checked={showArchived} onChange={e => { setShowArchived(e.target.checked); setSubmissionPage(0) }} />Mostrar arquivados</label></CardHeader><CardBody className="p-0">{visibleSubmissions.length === 0 ? <EmptyState title="Nenhum envio no período" description="Registre uma live concluída para iniciar a revisão." /> : <><div className="presenter-lives-desktop"><DataTable data={submissionSlice} rowKey={(item) => item.id} columns={[{ key: 'envio', header: 'Envio', render: (item) => <div><p className="font-semibold text-ink">{item.marca_nome ?? item.marca_descricao ?? 'Marca'}</p><p className="text-xs text-ink-muted">{formatDate(item.iniciado_em)}{item.motivo_devolucao ? ` · ${item.motivo_devolucao}` : ''}</p></div> }, { key: 'status', header: 'Status', render: (item) => <div className="flex flex-wrap gap-1.5"><Badge tone={submissionStatusTone(item.status)}>{item.arquivamento_status === 'confirmado' ? 'Arquivado' : item.arquivamento_status === 'solicitado' ? 'Aguardando sua resposta' : submissionStatusLabel(item.status)}</Badge>{submissionHasOfficialLiveTombstone(item) ? <Badge tone="neutral">Live excluída pelo gestor</Badge> : null}</div> }, { key: 'acoes', header: 'Ações', align: 'right', render: (item) => item.arquivamento_status === 'solicitado' ? <div className="flex flex-wrap justify-end gap-2"><Button variant="secondary" onClick={() => { respondArchive.reset(); setArchiveReason(''); setArchiveResponse({ item, acao: 'confirmar' }) }}>Confirmar arquivamento</Button><Button onClick={() => { respondArchive.reset(); setArchiveReason(''); setArchiveResponse({ item, acao: 'contestar' }) }}>Contestar devolução</Button></div> : item.status === 'devolvida' ? <div className="flex flex-wrap justify-end gap-2"><Button variant="secondary" icon={Pencil} onClick={() => setEditing(item)}>Corrigir</Button><Button icon={Send} isLoading={resend.isPending} onClick={() => resend.mutate(item.id)}>Reenviar</Button><Button variant="ghost" icon={Trash2} onClick={() => setCancelling(item)}>Cancelar envio</Button></div> : item.status === 'pendente' && submissionOwnerCanCancel(item) ? <div className="flex flex-wrap justify-end gap-2"><Button variant="ghost" icon={Trash2} onClick={() => setCancelling(item)}>Cancelar envio</Button></div> : item.status === 'aprovada' ? <CheckCircle2 className="h-5 w-5 text-[var(--success)]" aria-label="Aprovada" /> : item.status === 'cancelada' ? <XCircle className="h-5 w-5 text-ink-muted" aria-label="Cancelada" /> : null }]} /></div><SubmissionMobileList items={submissionSlice} resendPending={resend.isPending} onConfirm={(item) => { respondArchive.reset(); setArchiveReason(''); setArchiveResponse({ item, acao: 'confirmar' }) }} onContest={(item) => { respondArchive.reset(); setArchiveReason(''); setArchiveResponse({ item, acao: 'contestar' }) }} onEdit={setEditing} onResend={(id) => resend.mutate(id)} onCancel={setCancelling} /><TablePager page={submissionPage} total={visibleSubmissions.length} onChange={setSubmissionPage} /></>}</CardBody></Card>
