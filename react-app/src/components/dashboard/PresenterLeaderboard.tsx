...
import { rankingCommission, rankingGmv, rankingId, rankingImage, rankingLives, rankingName, rankingPedidos } from '../../utils/ranking'
...
export function getPresenterLeaderboardName(row: JsonRecord): string {
  return rankingName(row, 'apresentadora')
}
...
function normalizeRows(rows: JsonRecord[], limit?: number): PresenterRow[] {
  ...
  return visibleRows.map((row, index) => {
    ...
    return {
      key: rankingId(row, 'apresentadora') || String(index),
      name,
      initials: initialsFromName(name),
      avatarUrl: rankingImage(row, 'apresentadora'),
...
function positionTone(index: number) {
  if (index === 0) return {
    ...
    row: 'linear-gradient(90deg, oklch(0.58 0.13 62 / 0.22), oklch(0.58 0.13 62 / 0.08) 44%, transparent)',
  }
  if (index === 1) return {
    ...
    row: 'linear-gradient(90deg, oklch(0.72 0.02 250 / 0.18), oklch(0.72 0.02 250 / 0.07) 44%, transparent)',
  }
  if (index === 2) return {
    ...
    row: 'linear-gradient(90deg, oklch(0.62 0.11 48 / 0.16), oklch(0.62 0.11 48 / 0.06) 44%, transparent)',
  }
  return {
