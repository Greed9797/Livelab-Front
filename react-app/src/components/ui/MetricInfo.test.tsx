...
describe('MetricInfoPopover', () => {
  it('expõe role=tooltip e o conteúdo do glossário', () => {
    const html = renderToStaticMarkup(
      <MetricInfoPopover id="t1" metric="home.gmv_total" />,
    )
    const entry = METRIC_GLOSSARY['home.gmv_total']
    expect(html).toContain('role="tooltip"')
    expect(html).toContain('id="t1"')
    expect(html).toContain(entry.definicao)
    expect(html).toContain('routes/home.js:337')
  })

  it('mostra a memória de cálculo real quando o payload traz os números', () => {
    const html = renderToStaticMarkup(
      <MetricInfoPopover id="t2" metric="home.gmv_total" detail="R$ 10,00 = lives R$ 6,00 + vídeos R$ 4,00" />,
    )
