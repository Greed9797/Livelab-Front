...
export function LoadingState({ label = 'Carregando dados' }: { label?: string }) {
  return (
    <div role="status" aria-live="polite" className="flex min-h-48 items-center justify-center rounded-2xl border border-dashed border-line bg-surface/70 p-6 text-sm text-ink-muted">
      <Loader2 aria-hidden="true" className="mr-2 h-4 w-4 animate-spin text-brand" />
      {label}
    </div>
  )
}
...
export function ErrorState({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div role="alert" className="flex min-h-48 flex-col items-center justify-center rounded-2xl border border-[var(--danger-soft)] bg-[var(--danger-soft)] p-6 text-center">
