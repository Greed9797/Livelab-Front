...
test('keeps custom dates, pagination and presenter split connected across the live flow', async ({ page }) => {
  ...
  await liveRow.hover()
  const mobileOpen = liveRow.getByRole('button', { name: 'Abrir detalhes de Alto calçados', exact: true })
  if (await mobileOpen.isVisible()) await mobileOpen.click()
  else await liveRow.getByRole('button', { name: 'Abrir', exact: true }).click()
  ...
  await detail.getByText('Outras ações', { exact: true }).click()
  await detail.getByRole('button', { name: 'Dividir entre apresentadoras' }).click()

  const rateio = page.getByRole('dialog', { name: 'Divisão entre apresentadoras' })
  ...
  await expect(rateio.getByLabel('Apresentadora 1')).toHaveValue(sandyId)
  await expect(rateio.getByLabel('Apresentadora 2')).toHaveValue(cliceaneId)
})
