...
async function setup(page: Page, role = 'apresentadora', initialStatus = 'pendente', failCreateOnce = false) {
  page.on('pageerror', error => { throw error })
  const calls: Array<{ path: string; method: string; body?: Record<string, unknown> }> = []
  let items: Record<string, unknown>[] = initialStatus === 'empty' ? [] : [{ id: submission, apresentadora_id: presenter, apresentadora_nome: 'Ana', status: initialStatus, iniciado_em: '2026-09-05T12:00:00Z', encerrado_em: '2026-09-05T14:00:00Z', marca_id: brand, marca_nome: 'Marca Aurora', marca_descricao: 'Marca Aurora', cabine_id: cabin, cabine_nome: 'Cabine Norte', gmv_declarado: 200, pedidos_declarados: 2, motivo_devolucao: initialStatus === 'devolvida' ? 'Confira os pedidos' : null, versao: 1 }]
  await page.addInitScript(({ role, tenant }) => {
    ...
    localStorage.setItem('livelab.react.user', JSON.stringify({ id: '66666666-6666-4666-8666-666666666666', nome: 'Ana', papel: role, tenant_id: tenant, onboarding_completed: true }))
  }, { role, tenant })
  ...
  await page.route('**/v1/**', async route => {
    ...
    if (path === '/v1/portal/apresentadora/me') return route.fulfill({ json: {
      perfil: { id: presenter, nome: 'Ana', foto_url: null }, desempenho: { gmv_lives: 500, horas_live: 4, gmv_por_hora: 125, total_lives: 2, pedidos: 5 }, remuneracao: { fixo: 2850 },
