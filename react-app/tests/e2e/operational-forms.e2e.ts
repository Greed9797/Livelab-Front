...
test('Voltar do navegador exige escolha e mantém a edição ao continuar', async ({ page }, info) => {
  ...
  await page.getByRole('link', { name: 'Conteúdo', exact: true }).filter({ visible: true }).click()
  await page.getByRole('button', { name: 'Lives realizadas', exact: true }).click()
...
test('navegação tem nomes acessíveis, foco contido no celular e conteúdo sem transbordamento', async ({ page }, info) => {
  ...
  if (info.project.name === 'mobile-chrome') {
    ...
    await expect(menu.getByRole('link', { name: 'Conteúdo', exact: true })).toHaveAttribute('aria-current', 'page')
    ...
    await expect(nav.getByRole('link', { name: 'Conteúdo', exact: true })).toHaveAttribute('aria-current', 'page')
